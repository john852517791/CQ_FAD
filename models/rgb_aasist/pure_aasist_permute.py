...
class Model(nn.Module):
    ...
    def forward(self,x,train=False):
        ...
        # transform_img = self.conv2d(x.transpose(2,3)).flatten(2).transpose(1,2)
        bs, chennal, temporal, spectral = x.shape
        transform_img = x.transpose(2,3).reshape(bs,-1,spectral).transpose(1,2)
        # print(transform_img.shape)
        output,hid = self.classifier(transform_img)
