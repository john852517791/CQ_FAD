...
class Model(nn.Module):
    ...
    def forward(self,x,train=False):
        ...
        # transform_imgz = self.conv2d(transform_img.transpose(2,3)).flatten(2).transpose(1,2)
        bs, chennal, temporal, spectral = transform_img.shape
        transform_imgz = transform_img.transpose(2,3).reshape(bs,-1,spectral).transpose(1,2)
