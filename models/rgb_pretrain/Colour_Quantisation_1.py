...
from models.rgb_pretrain.UNeXt import UNext_S, UNext
...
class Model(nn.Module):
    def __init__(self,args = default_args()):
        super().__init__()
        out_channels = args.colour_out_channels
        self.num_colors = args.colour_num_colours
        self.Image_Encoder = UNext(num_classes=self.num_colors)
        self.conv_large = conv_embedding(3, out_channels)
        self.temperature = args.colour_temperature
        self.Colour_Cluster_Transformer = Colour_Cluster_Transformer(dim=out_channels, num_colours=self.num_colors,
                                                                     num_heads=args.colour_num_heads)
    ...
    def forward(self, x_RGB, training=True):
        ...
        # print(origin_probability_map.shape)
...
if __name__ == "__main__":
    ...
    img = torch.randn((128, 3, 256, 256)).to(device)
