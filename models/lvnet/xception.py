...
def xception(num_classes=1000, pretrained='imagenet', inc=3):
    ...
    if pretrained:
        ...
        # model.load_state_dict(model_zoo.load_url("/data8/wangzhiyong/project/fakeAudioDetection/rgbFAD/z_ref/Locate-and-Verify/model/xception-b5690688.pth"))
        lsd = torch.load("/data8/wangzhiyong/project/fakeAudioDetection/rgbFAD/z_ref/Locate-and-Verify/model/xception-b5690688.pth")
        for key in list(lsd.keys()):
            if 'pointwise.weight' in key:
                lsd[key] = lsd[key].unsqueeze(-1).unsqueeze(-1)
