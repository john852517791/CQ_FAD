...
if True:
    ...
    if not args.inference:
        ...
        trainer = L.Trainer(
            ...
            callbacks=[
                # dev损失无下降就提前停止
                EarlyStopping('batch_loss',patience=args.no_best_epochs,mode="min",verbose=True,log_rank_zero_only=True),
                # 模型按照最低val_loss来保存
                ModelCheckpoint(monitor='batch_loss',
                                save_top_k=1,
                                save_weights_only=True,mode="min",filename='best_model-{epoch:02d}-{batch_loss:.6f}'),
