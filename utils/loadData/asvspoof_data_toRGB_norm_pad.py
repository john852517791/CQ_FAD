...
class Dataset_ASVspoof2019_train(Dataset):
    def __init__(self, list_IDs, labels, base_dir,args,cut = 65600):
...
class Dataset_ASVspoof2019_devNeval(Dataset):
    def __init__(self, list_IDs, base_dir,cut = 65600):
        """self.list_IDs	: list of strings (each string: utt key),
        """
        self.list_IDs = list_IDs
        self.base_dir = base_dir
...
class Dataset_ASVspoof2019_eval(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # X, _ = sf.read(str(self.base_dir / f"flac/{key}.flac"))
        X, _ = sf.read(os.path.join(self.base_dir,f"flac/{key[1]}.flac"))
        if self.cut == 0:
            X_pad = X
        else:
            X_pad = pad(X, self.cut)
        ...
        x_inp = Tensor(rgb_tensor)
        return x_inp, key[1],key[3],key[4]
